...
1.  Never set, export, or add `ALLOW_REAL_CLAUDE` to any env file, npm script,
...
- No Postgres functions, no triggers, no `.rpc()`, and no raw SQL in `src/`.
  ...
  (`usage/aggregate.ts` and `usage/allowance.ts` already cite this rule in
  comments as though CLAUDE.md contained it — it now does.)
...
- Prompt caching is wired on both `/prompts` and `/shots` but is inert: the
...
- **Specs assert against their own project's rows, not global per-user totals.**
  ...
  created, never to "all of this user's rows." Five tests, across three spec files,
  assert something that's *inherently* a global-per-user fact and so must keep their
  own fresh, never-attempted `createTestSession()` user instead of sharing
  `primary`/`secondary` — do not "fix" these by switching them to a fixed user:
...
`generations`, `usage`, `shot_dialogue`. All RLS-protected. Two tables
