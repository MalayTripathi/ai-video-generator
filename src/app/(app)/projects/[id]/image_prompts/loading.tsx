import { PromptCardSkeleton } from './_components/prompt-skeleton'

function Bar({ className }: { className: string }) {
  return <span className={`inline-block rounded-[3px] bg-skeleton-base ${className}`} />
}

// Streamed skeletons in the real card's shape - never a spinner over an empty page
// (canvas 14B). The chrome mirrors the Workbench's loading state; only the content pane
// is Step 3's own.
export default function ImagePromptsLoading() {
  return (
    <>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-rc-sm overflow-hidden px-rc-md py-rc-md">
          <div className="flex flex-none items-center justify-between">
            <Bar className="h-3 w-40" />
            <div className="flex gap-[10px]">
              <span className="block h-[34px] w-36 rounded-control bg-skeleton-base" />
              <span className="block h-[34px] w-40 rounded-control bg-skeleton-base" />
            </div>
          </div>
          <div className="flex flex-col gap-[10px]">
            <PromptCardSkeleton badge={74} line2={92} line3={61} />
            <PromptCardSkeleton badge={46} line2={86} line3={44} />
            <PromptCardSkeleton badge={62} line2={95} line3={70} />
          </div>
        </div>
      </div>
